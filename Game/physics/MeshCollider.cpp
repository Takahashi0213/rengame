...
void MeshCollider::CreateFromSkinModel(const SkinModel& model, const CMatrix* offsetMatrix, const CVector3 scl, bool biasFlag) {
	...
	m_meshShape = std::make_unique<btBvhTriangleMeshShape>(m_stridingMeshInterface.get(), true);
	m_meshShape->setLocalScaling(scl);
